...
3. **Backend swap** — When an alternative backend is configured, the layout
   connects it asynchronously in `onMount` and calls `setDelegate()` on the
...
- Future implementations (e.g. PowerSyncDataService) will plug into the same
  interface, enabled via environment variable toggle.
